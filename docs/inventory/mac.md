...
**Aliases** — shared: `cls`, `sail`, `jqd64`, `genpass`, `ll`/`la`, `garage` (same hardcoded kube context + pod — per-machine override material), `ccusage` (`npx` here vs `pnpx` on WSL). Mac-only, all work-scoped: `aofroot` (cd to work project), `aof-pdfinfo` (work jar path), and `aoftunnel` — see §3.3, **this alias embedded a tunnel credential inline** (removed same day this inventory flagged it; the credential was already invalid).
...
|---|---|---|
| `~/.zshrc` | shell rc | core artifact — **after stripping the `aoftunnel` secret and work aliases** (§3.3) |
...
- Work aliases (`aofroot`, `aof-pdfinfo`) and `garage` need the per-machine/untracked override mechanism (`aoftunnel`, which embedded a credential, was removed on inventory day).
...
**Secrets strategy (#6)**
- The (removed) `aoftunnel` alias proved secrets can live *inside* shell rc files, not just beside them — the strategy must cover rc content, not only excluded files.
