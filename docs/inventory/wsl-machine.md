...
| `~/.secrets/ansible-vault-pass` | Ansible Vault password (referenced by the global agent rules) |
...
**Secrets vault (#11)**
- Loose secret files sitting directly in `$HOME` (2 env backups, a project values YAML) plus `~/.secrets/ansible-vault-pass` have no policy today.
